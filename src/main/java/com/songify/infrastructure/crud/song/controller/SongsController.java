...
import com.songify.domain.crud.song.SongCrudFasade;
...
import static com.songify.infrastructure.crud.song.controller.SongControllerMapper.mapFromSongToPartiallyUpdateSongResponseDto;
...
public class SongsController {
    ...
    private final SongCrudFasade songFasade;

    @GetMapping
    public ResponseEntity<GetAllSongsResponseDto> getAllSongs(Pageable pageable) {
        List<SongDto> allSongs = songFasade.findAll(pageable);
    ...
    public ResponseEntity<GetSongResponseDto> getSongsById(@PathVariable Long id, @RequestHeader(required = false) String requestId) {
        log.info(requestId);
        SongDto songEntity = songFasade.findById(id);
